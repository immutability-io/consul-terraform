...
resource "null_resource" "issue-certificate" {
    provisioner "local-exec" {
        command = <<EOT
        VAULT_TOKEN=${var.vault_token} vault write -address=${var.vault_addr} -format=json vault_intermediate/issue/web_server common_name="${var.common_name}" alt_names="${var.alt_names}" ip_sans="${var.ip_sans}" ttl=720h > ./tmp.json
EOT
    }
    provisioner "local-exec" {
